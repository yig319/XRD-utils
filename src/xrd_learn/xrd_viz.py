...
def plot_xrd(inputs, labels, title='XRD Scan', xrange=None, yrange=None, diff=1e3, yscale='log', fig=None, ax=None, figsize=(6,4), xlabel=r"2$\Theta$ [°]", ylabel='Intensity [a.u.]', legend_style='legend', colors=None, text_offset_ratio=None, grid=False, pad_sequence=None):
    ...
    for i, (X, Y) in enumerate(zip(Xs, Ys)):
        ...
        if callable(colors):
            plot_color = colors(i % getattr(colors, "N", 10))
        else:
            plot_color = colors[i % len(colors)]
            
        if legend_style == 'legend':
            ax.plot(X, Y, label=labels[i], color=plot_color)
        elif legend_style == 'label':
            line, = ax.plot(X, Y, color=plot_color)
